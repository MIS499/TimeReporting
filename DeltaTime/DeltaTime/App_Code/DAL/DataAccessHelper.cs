...
namespace ASPNET.DataAccessLayer {
  public class DataAccessHelper {
    public static DataAccess GetDataAccess() {
      ...
      if (String.IsNullOrEmpty(dataAccessStringType)) {
        ...
        Type tp = Type.GetType("ASPNET.DataAccessLayer.DataAccess");
        if (!tp.IsAssignableFrom(dataAccessType)) {
          throw (new ArgumentException("DataAccessType does not inherits from ASPNET.DataAccessLayer.DataAccess "));
